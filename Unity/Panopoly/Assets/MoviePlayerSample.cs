...
public class MoviePlayerSample : MonoBehaviour {
	...
	public String filename = "/sdcard/oculus/Movies/Trailers/Night_of_the_living_Dead.mp4";
	...
	string GetVideoFilename()
	{
		...
		try
		{
			...
			{
				Debug.Log ("www error: " + StreamingFile.error );
				return null;
			}
			
			Debug.Log("Writing to persistent: " + PersistFilename );
			System.IO.File.WriteAllBytes ( PersistFilename, StreamingFile.bytes );
			
			if (!System.IO.File.Exists(PersistFilename))
			{
				Debug.Log ("File does not exist after copy: " + PersistFilename );
				return null;
			}
	...
	AndroidJavaObject StartVideoPlayerOnTextureId( int textureId ) {
		...
		mediaPlayer.Call( "setDataSource", VideoFilename );
