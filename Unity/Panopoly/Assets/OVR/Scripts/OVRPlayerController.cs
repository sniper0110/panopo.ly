...
public class OVRPlayerController : MonoBehaviour
{
	...
	// normally this should be zero because the HMD is rotating the head,
	// but during debugging it is often necessary to rotate the view without
	// wearing an HMD.
	...
	public void Update()
	{
		...
		float motorDamp = (1.0f + (Damping * OVRDevice.FrameRate * Time.deltaTime));
		MoveThrottle.x /= motorDamp;
		MoveThrottle.y = (MoveThrottle.y > 0.0f) ? (MoveThrottle.y / motorDamp) : MoveThrottle.y;
		MoveThrottle.z /= motorDamp;

		moveDirection += MoveThrottle * OVRDevice.FrameRate * Time.deltaTime;
		
		// Gravity
		if (Controller.isGrounded && FallSpeed <= 0)
			FallSpeed = ((Physics.gravity.y * (GravityModifier * 0.002f)));	
		else
			FallSpeed += ((Physics.gravity.y * (GravityModifier * 0.002f)) * OVRDevice.FrameRate * Time.deltaTime);	

		moveDirection.y += FallSpeed * OVRDevice.FrameRate * Time.deltaTime;
		...
		if (predictedXZ != actualXZ)
			MoveThrottle += (actualXZ - predictedXZ) / (OVRDevice.FrameRate * Time.deltaTime);
	...
	/// <summary>
	/// Resets the player look rotations when the device orientation is reset.
	/// </summary>
