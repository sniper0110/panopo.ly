...
public class OVRMainMenu : MonoBehaviour
{
	...
	void Start()
	{
		...
		OVRDevice.Attached += UpdateDeviceDetectionMsgCallback;
	...
	void UpdateSelectCurrentLevel()
	{
		...
		if((Scenes.Length != 0) && 
		   ((OVRGamepadController.GPC_GetButton(OVRGamepadController.Button.A) == true) ||
	...
	bool ShowLevels()
	{
		...
		if(OVRGamepadController.GPC_GetButton(OVRGamepadController.Button.Start) == true)
			curStartDown = true;
	...
	void UpdateDeviceDetectionMsgCallback(object sender, EventArgs args)
	{
		var attachedArgs = (OVRDevice.AttachedArgs)args;
		bool attached = attachedArgs.IsAttached;
		Device device = attachedArgs.Device;
